...
public class RangeAttack : MonoBehaviour
{
    ...
    [Header("+++BULLET+++")]
    public Projectile Projectile, GunProjectile;
     int normalDamage = 30;
    ...
    //public bool GunFire(Vector3 _direction)
    //{
    //    if ((GameManager.Instance.isTestLevel || GlobalValue.Bullets > 0) && Time.time > nextFire)
    //    {
    //        nextFire = Time.time + fireRate;
    ...
    //        //    GlobalValue.Bullets--;
    //        direction = _direction;
    //        StartCoroutine(GunDelayAttack(fireDelay, false));
    //        return true;
    //    }
    //    else
    //        return false;
    //}
    //IEnumerator GunDelayAttack(float time, bool powerBullet)
    //{
    //    yield return new WaitForSeconds(time);

    //    Vector2 firePoint = FirePoint.position;

    //    var projectile = Instantiate(GunProjectile.gameObject, firePoint, GunProjectile.gameObject.transform.rotation).GetComponent<Projectile>();
    //    projectile.Initialize(gameObject, direction, Vector2.zero, powerBullet, false, normalDamage, bulletSpeed);
    //    projectile.gameObject.SetActive(true);
